...
- **GitHub Actions** (`.github/workflows/ci.yml`): on each push/PR to `main`/`master`, runs `npm ci`, **`npm run ops:verify`**, **`npm run typecheck`**, **`npm run lint`** (`eslint.config.mjs`), **`npm run build:production`**, **`npm audit --audit-level=high`**, and **`docker build`**. You can also run it manually (**Actions → CI → Run workflow**).
- **Production go-live:** follow **`OPERATIONS_RUNBOOK.md`** (health, CORS, backups, drills). Future features (card payments, CAPTCHA, etc.): **`docs/ROADMAP.md`**.
