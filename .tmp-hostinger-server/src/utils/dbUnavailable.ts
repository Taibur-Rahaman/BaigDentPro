...
export function sendDatabaseUnavailable(res: Response): void {
  res.status(503).json({
    error:
      'Database is not reachable. Start PostgreSQL (from repo root: `docker compose up -d`), then run `cd server && npx prisma db push && npm run db:seed`. Or set DATABASE_URL to your hosted Postgres.',
  });
}
